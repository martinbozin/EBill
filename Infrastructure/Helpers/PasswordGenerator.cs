...
namespace EBills.Infrastructure.Helpers
{
    public class PasswordGenerator
    {
        ...
        //private char[] pwdCharArray = "abcdefghijkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ123456789~!@#$%^&*()-_=+[]{}\\|;:,<.>/?".ToCharArray();
        private char[] pwdCharArray = "abcdefghijkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ123456789!@#$%&".ToCharArray();


        public static byte[] HashString(string email, string password)
        {
            try
            {
                var salt = "EKLURYKEYRJK&^76575672343hgg76&^^&^%$%%^" + email;
                var provider = new SHA1CryptoServiceProvider();
                var encoding = new UnicodeEncoding();
                return provider.ComputeHash(encoding.GetBytes(password + salt));
            }
            catch (Exception ex)
            {
