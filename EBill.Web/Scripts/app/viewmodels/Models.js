...
NS.ViewModels.UserViewModel = function () {
    ...
    self.isUpravenInspektor = function () {
        for (var i = 0; i < this.Roles().length; i++) {
            var role = this.Roles()[i];
            if (role.RoleName == 'UpravenInspektor') {
                return true;
            }
        }
        return false;
...
NS.ViewModels.RequestHistoryModel = function () {
    ...
    self.mapFromJson = function (data) {
        ...
        self.State(window.languagePack["EBills.Web.States.Name." + data.State + ""]);
        if (data.Modified != null) {
            var fromJsonDate1 = NS.Utils.Date.parseDateFromJson(data.Modified);
...
NS.ViewModels.RequestViewModel = function () {
    ...
    self.mapFromJson = function (data) {
        ...
        self.StateTitle(window.languagePack["EBills.Web.States.Name." + data.StateTitle + ""]);
        self.StateTag(data.StateTag);
        self.PublicGroup(window.languagePack["EBills.Web.PublicStates.Name." + data.PublicGroup + ""]);

        self.ConstructionTypeId(data.ConstructionTypeId);
        self.ConstructionTypeName(window.languagePack["EBills.Web.ConstructionType.Name." + data.ConstructionTypeName + ""]);
        ...
        self.FreeUtilities(window.languagePack["EBills.Web.Request.FreeUtilities." + data.FreeUtilities + ""]);
...
NS.ViewModels.RequestNotificationViewModel = function () {
    ...
    self.MapFromJson = function (data) {
        ...
        if (data.ResponseStatus == 2) {
            self.ResponseStatus(languagePack['Scripts.app.viewmodels.Models.js.Negativno']);//"Негативно"
        }
...
NS.ViewModels.Request2UserNotificationItemViewModel = function () {
    ...
    self.mapFromJson = function (data) {
        if (data.NotificationTypeName == "IncompleteRequest") {
            self.NotificationTypeName(languagePack['Scripts.app.viewmodels.Models.js.NekompletnoBaranje']);//"Некомплетно барање"
        }
        else if (data.NotificationTypeName == "PayingUtilities") {
            self.NotificationTypeName(languagePack['Scripts.app.viewmodels.Models.js.PlakanjeNaKomunalii']);//"Плаќање на уредување на градежно земјиште"
        }
        else if (data.NotificationTypeName=="SMS") {
            self.NotificationTypeName(languagePack['Scripts.app.viewmodels.Models.js.SmsPoraka']);//"СМС порака"
        }
        self.Comment(data.Comment);
        if (data.Active == true) {
            self.Active(languagePack['Scripts.app.viewmodels.Models.js.Aktivna']);//"Активна"
        }
        else if (data.Active == false) {
            self.Active(languagePack['Scripts.app.viewmodels.Models.js.Neaktivna']);//"Неактивна"
        }
