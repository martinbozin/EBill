$.widget("NS.NewRequestAttachDocumentWidget",
{
    options: {
        templateId: 'new-request-attach-document',
        autoClose: false
    },
    _init: function () {
    ...
    _initAplicationUIActions: function () {
        ...
        if ($.browser.msie) {
            $("#ieFileUpload").show();
            $('#ieFileUpload').die('change').live('change', function () {
                var newValue = $(this).val();
                if (viewModel.DocumentType().AllowedExtensions() != null) {
                    var ext = newValue.substr(newValue.lastIndexOf('.') + 1);
                    if (viewModel.DocumentType().AllowedExtensions().indexOf(ext) != -1) {
                        var item = new NS.ViewModels.UploadDocumentViewModel();
                        item.FileName(extractFileName(newValue));
                        item.FileInputElement('ieFileUpload');
